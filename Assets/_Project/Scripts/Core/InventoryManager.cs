...
namespace EchoesOfArcadia.Core
{
    ...
    public class InventoryManager : MonoBehaviour
    {
        ...
        public void UseItem(ItemData item, Battle.BattleUnit target)
        {
            ...
            switch (item.effect)
            {
                case ItemEffect.HealHP:
                    ...
                    break;
                case ItemEffect.HealSP:
                    target.RestoreSP(item.effectValue);
                    break;
                case ItemEffect.HealHPSP:
                    target.Heal(item.effectValue);
                    target.RestoreSP(item.effectValue / 2);
                    break;
                case ItemEffect.ReviveAlly:
                    if (!target.IsAlive)
                        target.Revive(item.effectValue);
                    break;
            }
