...
## 現在のフェーズ
**3Dフィールド探索実装済み** — プリミティブ3Dワールド（Capsule=キャラ/Cube=建物）で歩き回れる状態
...
1. ScriptableObjectによるデータ駆動化（キャラ・スキル・敵）
   理由: ハードコードから脱し、バランス調整とデザイナーワークフローを整える。
2. 絆ランク2以降の会話JSON拡充
   理由: 社会リンクの感情密度が5/10と最低。テーマの核心なので急ぎたい。
3. 潜行開始演出の強化（アプリ「未読」→水紋→世界裏返り）
   理由: DESIGN.md演出7。異界への没入感を決定づける。
4. ミニマップまたはコンパスUI（3Dフィールド用）
   理由: 3Dフィールドで方向感覚を失いやすい。探索の快適性向上。
