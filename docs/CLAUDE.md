...
- 時間システムの機能性:  7.5/10（昼休みAP追加でP4相当の時間管理に近づいた。CalendarEvent自動会話起動も実装）
...
- 総合:                  8.0/10（前回7.8→8.0。時間システムの機能性改善で全体的な完成度UP）
前回からの変化: ScriptableObject データ整備（CharacterData/EnemyData/AbilityData）、CalendarEvent自動会話起動（9本のストーリーイベントJSON）、rank1 JSON全キャラ追加、昼休み行動枠（Lunch APシステム）、絆会話の bondBonus 即時適用修正。
...
## 次回セッションの最優先タスク
1. ~~ScriptableObjectによるデータ駆動化~~: 完了（CharacterData/EnemyData/AbilityData実装済み）
2. ~~CalendarEvent発生時の会話/シーン自動遷移~~: 完了（9本のストーリーイベントJSON + 自動起動実装済み）
3. ~~絆ランクアップ後に次のランクのダイアログを自動選択する修正~~: 確認済み（StartBondDialogue()は現在ランクのJSONを読む正しい設計。問題なし）
4. **デュアルナレーター（DESIGN.md 9-1）の実装**
   理由: 第3幕解放予定の主人公専用能力。語り手スワップを2体同時保持に拡張。SP消費1.5倍の制約と属性相性マトリクス。
5. **潜行開始演出の強化（アプリ「未読」→水紋→世界裏返り）**
   理由: DESIGN.md 必須演出7番。最も印象的な演出のひとつでまだ仮状態。
6. **昼休みのバリエーション拡充**
   理由: 現在はキャラ会話3種のみ。「購買」（消耗品入手）・「屋上」（静けさ+静謐な演出）・「職員室」（知識アイテム）等。
7. **深夜潜行オプション（DESIGN.md 9-2）**
   理由: 静けさ≥4で解放。翌日APゼロのリスクと「今夜潜りたい」切迫感のトレードオフ。
