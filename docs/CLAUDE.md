...
## 現在のフェーズ
**全3キャラ（灯里/律/蓮）絆ランク1-10 JSON完成 + ダンジョン探索ループ接続済み**
...
- ストーリーの引力:      8/10（設計上＋全絆会話で感情の弧が完結）
- 総合:                  7.8/10（前回7.5→7.8。社会リンク完成でペルソナの核心に近づいた）
前回からの変化: 全3キャラのランク6-10 JSON完成（合計15本）。律のKOTOBAシステム/蓮の祖母との別れ/灯里の美月のお母さんへの手紙。各キャラの感情アークが完結。ダンジョン探索→戦闘→帰還ループも接続完了。
...
## 次回セッションの最優先タスク
1. ScriptableObjectによるデータ駆動化（キャラ・スキル・敵）
   理由: ハードコードから脱し、CharacterData/EnemyData/AbilityDataを整備することで外部JSONとの分離が実現する。
2. CalendarEvent発生時の会話/シーン自動遷移の実装
   理由: 月次イベント（満月の母の記憶、学園祭等）が自動トリガーされれば第1幕ストーリーが実際に流れ始める。
3. 絆ランクアップ後に次のランクのダイアログを自動選択する修正
   理由: 現状StartBondDialogue()はrank番号のJSONを直接読むため、ランクアップ後の次回会話が正しく選択されるか要確認。
